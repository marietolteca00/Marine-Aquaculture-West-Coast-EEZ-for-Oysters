...
tm_shape(eez_sf) +
  ...
  tm_scale_bar(
    position = c("left","bottom"), text.size=0.35
  ) +
  tm_compass(
    type = "arrow",
...
suit_func <- function(
  ...
  mutate(suitable_area_km2_new = as.numeric(suitable_area_km2_new)) %>% 
  as_tibble() %>% 
  arrange(desc(suitable_area_km2_new))
  ...
  suit_func_map <- 
    tm_shape(eez_joined) +
      ...
      tm_scale_bar(position = c("left", "bottom"), text.size = 0.35) +
...
# Testing another Species for fun
suit_func(
  sst_raster = sst_mean,
  depth_raster = depth_resampled,
  eez_sf = eez_sf,
